#!/bin/bash
project_name=myDafy
current_path=/var/www/http/mydafy/current/
...
if [ $# -gt 1 ];then
    ...
    if [ $ans = 'yes' -o $ans = 'y' ];then
        ...
        ssh root@x.x.x.1 "cp -r /home/workspace/myDafy/dist /home/workspace/myDafy/dist_backup_`date +'%Y%m%d%H%M%S'`"
        exit_status "备份生产环境46即有钱包代码"


        ssh root@x.x.x.2 "cp -r /home/workspace/myDafy/dist /home/workspace/myDafy/dist_backup_`date +'%Y%m%d%H%M%S'`"
        exit_status "备份生产环境147即有钱包代码"

        sleep 2

        rsync -zrt -e 'ssh' --exclude='.git' --exclude='*.org' \
            --exclude-from=${excludefiles} --delete ${current_path}/dist/ root@x.x.x.1:/home/workspace/myDafy/dist/
        exit_status "于`date +'%Y%m%d%H:%M:%S'`同步推送即有钱包至46生产环境"

        rsync -zrt -e 'ssh' --exclude='.git' --exclude='*.org' \
            --exclude-from=${excludefiles} --delete ${current_path}/dist/ root@x.x.x.2:/home/workspace/myDafy/dist/
